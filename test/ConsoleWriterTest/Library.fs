...
let ``The writer should be able to handle simple outputs``() =
  let cw = (new Forest.ConsoleWriter.ConsoleWriter("nothing", false) :> Forest.ITarget)
  withConsole(fun _ ->
    cw.Write "LogName" Forest.Log "Test Log"
  )(fun output -> output |> should equal "\u001b[0;34m[LogName] LOG: Test Log\u001b[0m\r\n")
  withConsole(fun _ ->
    cw.Write "LogName" Forest.Warn "Test Warn"
  )(fun output -> output |> should equal "\u001b[0;33m[LogName] WARN: Test Warn\u001b[0m\r\n")
  withConsole(fun _ ->
    cw.Write "LogName" Forest.Error "Test Error"
  )(fun output -> output |> should equal "\u001b[0;31m[LogName] ERROR: Test Error\u001b[0m\r\n")